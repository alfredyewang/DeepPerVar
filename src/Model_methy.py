...
class SeqSubNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc1 = nn.Linear(19200, 5120)
        self.bn1 = nn.BatchNorm1d(num_features=5120)
...
def test(model, device, x_test):
    ...
    x_test = x_test.reshape(x_test.shape[0], 4, 1, 1000)
    x_test = torch.from_numpy(x_test).float()
    x_clinical = torch.from_numpy(x_clinical).float()

    with torch.no_grad():
        total_batch = int(np.shape(x_test)[0] / batch_size_test)

        for i in range(total_batch + 1):
            if i == total_batch:
                x_batch, x_cli = x_test[i * batch_size_test:x_test.shape[0]], \
                                 x_clinical[i * batch_size_test:x_test.shape[0]]
            else:
                x_batch, x_cli = x_test[i * batch_size_test:i * batch_size_test + batch_size_test], \
                                 x_clinical[i * batch_size_test:i * batch_size_test + batch_size_test]
...
def main(x_test, model_dir):
    ...
    model.load_state_dict(torch.load('{}/DeepPerVar_methy.pt'.format(model_dir), map_location='cpu'))
